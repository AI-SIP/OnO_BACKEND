 ...
 *
 * <p>자리의 {@code layerOrder} 는 <b>기본값</b>이다. 아이템이 {@code cosmetic_item.layer_order} 로
 * 덮어쓸 수 있다. {@link #BAG} 이 그래서 하나로 합쳐질 수 있었다. 등에 메는 것은 본체(300)보다 뒤에
 * 깔려야 하고 앞으로 메는 것은 옷(400) 위에 올라가야 하는데, 층이 다르다는 이유만으로 자리를
 * 둘로 나누면 사용자에게 설명할 수 없는 탭이 하나 더 생긴다.
 *
...
public enum CosmeticSlot {
    ...
    /**
     * 가방. 등에 메는 것과 앞으로 메는 것이 한 자리다.
     *
     * <p>둘을 자리로 갈라 뒀었는데(BACK 200 / BAG 450) 각각 2개·3개뿐이라 탭을 나눌 만큼이
     * 아니었고, 사용자에게 "왜 가방 자리가 둘이지" 를 설명해야 했다.
     *
     * <p>대신 그리는 층을 아이템이 덮어쓴다. 자리 기본값은 450(옷 위)이고,
     * 등에 메는 {@code back_backpack_*} 둘만 {@code cosmetic_item.layer_order} 에 200 을 직접
     * 갖고 있어 개구리 본체(300)보다 뒤에 깔린다. 자리 하나에 층이 둘인 것은 여기뿐이다.
     */
