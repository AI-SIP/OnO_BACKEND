 ...
 * 미션 보상 지급.
 *
 * <p><b>XP 가 들어오는 유일한 경로다.</b> 예전에는 행동할 때마다 {@code MissionLogService} 가 자동으로
 * 경험치를 주고 하루 200점 상한이 그것만 눌렀다. 같은 XP 가 두 경로로 들어오면서 화면의 숫자와
 * 실제 증가량이 어긋났고, 출처에 따라 상한 적용도 달랐다. 자동 적립을 걷어내 지급을 여기 하나로 모았다.
 * 하루 상한은 미션 설계가 대신한다. 일일 6종을 다 받아도 75, 주간 4종을 다 받아도 360 이다.
 *
