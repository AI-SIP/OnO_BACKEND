 ...
 *
 * <p>자동 적립과 이 경로는 <b>한 행동에 둘 중 하나만</b> 열린다. 두 경로가 같은 행동으로 함께 돌면
 * 같은 XP 를 두 번 받는 이중 지급이다. 어느 쪽이 열릴지는 {@link LegacyAccrualPolicy} 가 요청마다 정한다.
 * 자동 적립이 도는 요청에서는 진행도가 오르지 않아 받을 미션이 생기지 않고, 미션을 받을 수 있는 앱의
 * 요청이나 비상 스위치({@code ono.mission.legacy-accrual.enabled})가 꺼진 동안에는 여기가 XP 가 들어오는
 * 유일한 경로다.
 *
