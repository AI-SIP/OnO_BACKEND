...
### 자유롭게 해도 되는 것
- 읽기 전용 조사: 파일 읽기, 검색, `git status/diff/log/show`, 영향 범위 분석.
- 로컬 검증: 테스트, 빌드, 정적 검사, dev 환경 확인.
- 브랜치 확인, 커밋 메시지 정리, `/commit` 요청에 따른 실제 커밋 수행. push는 `/pr`에서 원격 브랜치 생성 또는 미push 커밋 반영이 필요할 때 수행한다. merge는 사용자가 명시적으로 요청한 경우에만 수행한다.
...
- Docker 배포, 인프라 설정 변경, 외부 발송, 되돌리기 어려운 작업은 실행 전 영향 범위와 rollback 방법을 설명하고 확인받는다.
